/**
 * WORKRAIL v2.3 Firebase Configuration Proxy
 *
 * Cloud Function (2nd gen) to securely distribute Firebase API key without exposing
 * it in client code.
 * Deployed to: https://us-central1-workrail-solenis.cloudfunctions.net/getFirebaseConfig
 *
 * SETUP INSTRUCTIONS:
 * 1. Deploy this function to Firebase Cloud Functions (2nd gen):
...
/**
 * Firebase Config Endpoint (2nd gen)
  ...
  (req, res) => {
    cors(req, res, () => {
      ...
      try {
        ...
        logger.info("Firebase config requested", {
          ip: clientIp,
        ...
        logger.error("Firebase config endpoint error", { error });
        return res.status(500).json({
          error: "Server error",
...
/**
 * Log cleanup function (2nd gen scheduled)
...
exports.cleanupRequestLog = onSchedule(
  ...
  () => {
    ...
    logger.info("Request log cleanup completed", {
